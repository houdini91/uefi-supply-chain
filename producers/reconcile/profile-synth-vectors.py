...
RELOC_VA, RELOC_RAW, RELOC_SIZE = 0x2000, 0x300, 0x100
...
def build_pe(image_base, pe_plus=True, with_reloc=True, reloc_type=None,
    ...
    struct.pack_into("<I", buf, opt + 64, checksum)        # CheckSum
    ...
    struct.pack_into("<I", buf, opt + 60, 0x200)           # SizeOfHeaders   pefile rejects
    ...
    if with_reloc:                                         # one block, two entries
        struct.pack_into("<II", buf, RELOC_RAW, TEXT_VA, nreloc)
...
def check(path):
    ...
    for v in want["vectors"]:
        ...
        bad += 0 if ok else 1
    print(f"\n{'PASS' if not bad else 'FAIL'}  {len(want['vectors'])} synthetic vectors, {bad} failing")
